...
function updateNodeElement(newElement, virtualDOM) {
  ...
  var oldProps = oldVirtualDOM.props || {}; // 文本节点

  if (virtualDOM.type === "text") {
    if (newProps.textContent !== oldProps.textContent) {
      // 父节点不一样，直接添加节点  比如p->div
...
var getFirstTask = function getFirstTask() {
   ...
   * 从任务队列中获取任务
   */
  var task = taskQueue.pop(); // executeTask执行完后，没有返回值。 所以subTask变为undefined。
  // 更新组件状态的时候，就会重新getFirstTask

  if (task.from === "class_component") {
    var root = Object(_Misc__WEBPACK_IMPORTED_MODULE_1__["getRoot"])(task.instance); // 记录新的state到fiber对象上
...
var executeTask = function executeTask(fiber) {
  /**
   * 构建子级fiber对象
   * children是createElement的VirtualDom
   */
